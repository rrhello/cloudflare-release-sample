(function () {
  ...
  var releaseURL = currentScript && currentScript.src ? currentScript.src : "";
  var lastHistoryURL = window.location.href;

  window.rccReleaseURL = releaseURL;
  if (window.console && console.info) {
    console.info("[RCC] Loaded release URL:", releaseURL);
  }
  ...
  function detectSurface() {
    if (/^\/courses\/products\/[^/]+/.test(window.location.pathname)) {
      return "portal-home";
    }
  ...
  function patchHistoryLoading() {
    ...
    ["pushState", "replaceState"].forEach(function (method) {
      ...
      window.history[method] = function () {
        ...
        trace("history:" + method, {
          ...
          customLoader: useCustomLoader
        });
        if (
          routeChanged &&
          useCustomLoader &&
          !document.documentElement.classList.contains("rcc-release-loading")
        ) {
          showLoadingVeil("history:" + method);
        } else if (routeChanged && useCustomLoader) {
          trace("loader:reuse", { reason: "history:" + method });
        } else if (routeChanged) {
          trace("loader:native-skeleton", { reason: "same-group:" + method });
        }
    ...
    window.addEventListener("popstate", function () {
      var targetURL = window.location.href;
      var useCustomLoader = shouldUseCustomLoader(lastHistoryURL, targetURL);
      trace("history:popstate", { customLoader: useCustomLoader });
      if (useCustomLoader) {
        showLoadingVeil("history:popstate");
      } else {
        trace("loader:native-skeleton", { reason: "same-group:popstate" });
      }
      lastHistoryURL = targetURL;
      window.setTimeout(initialize, 0);
    });
  }
  ...
  function groupSlugFromURL(value) {
    var match;
    try {
      match = new URL(value, window.location.href).pathname.match(
        /\/communities\/groups\/([^/?#]+)/
      );
    } catch (error) {
      return "";
  ...
  function isCommunityURL(value) {
    try {
      return /^\/communities(?:\/|$)/.test(new URL(value, window.location.href).pathname);
    } catch (error) {
      return false;
    }
  }

  function shouldUseCustomLoader(fromURL, toURL) {
    var fromCommunity = isCommunityURL(fromURL);
    var toCommunity = isCommunityURL(toURL);
    var fromSlug;
    var toSlug;

    if (fromCommunity !== toCommunity) return true;
    if (!fromCommunity && !toCommunity) return true;

    fromSlug = groupSlugFromURL(fromURL);
    toSlug = groupSlugFromURL(toURL);
    ...
    function startNavigationFromIntent(event) {
      var target = event.target && event.target.closest
        ? event.target.closest(
          '.rcc-source-group-button, .rcc-source-group-rail [role="button"]'
        )
        : null;

      if (!target || target.hasAttribute("aria-disabled")) return;
      if (event.type === "click" && event.detail !== 0) return;
      if (window.__rccSkipNextNavigationIntent) {
        window.__rccSkipNextNavigationIntent = false;
        trace("navigation:intent-skipped", {
          event: event.type,
