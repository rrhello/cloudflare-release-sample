(function () {
  ...
  function findNativeControl(action) {
    ...
    if (action === "notifications") {
      notificationPanel = document.querySelector(".nav-container .notification-height");
      if (notificationPanel && notificationPanel.parentElement) {
        if (notificationPanel.parentElement.previousElementSibling) {
          return notificationPanel.parentElement.previousElementSibling;
        }
      }
  ...
  function closeBentoModal() {
    ...
    if (!modal) return;
    restore = modal._rccRestore;
    modal.remove();
    document.body.classList.remove("rcc-bento-modal-open");
    if (typeof restore === "function") restore();
  }

  function openBentoModal(title, content, restore) {
    var modal = createElement("div", "rcc-bento-modal");
    var backdrop = createElement("button", "rcc-bento-modal__backdrop");
    var dialog = createElement("section", "rcc-bento-modal__dialog");
    var header = createElement("header", "rcc-bento-modal__header");
    var heading = createElement("h2", "rcc-bento-modal__title", title);
    var close = createElement("button", "rcc-bento-modal__close", "×");
    var body = createElement("div", "rcc-bento-modal__body");

    closeBentoModal();
    modal.id = MODAL_ID;
    modal._rccRestore = restore;
    backdrop.type = "button";
    backdrop.setAttribute("aria-label", "Close " + title);
    ...
    backdrop.addEventListener("click", closeBentoModal);
    close.addEventListener("click", closeBentoModal);
    document.body.appendChild(modal);
    document.body.classList.add("rcc-bento-modal-open");
    close.focus();
  }

  function openAccountModal() {
    var actions = createElement("div", "rcc-bento-modal__actions");
    var manage = createElement("button", "rcc-bento-modal__action", "Manage Your Account");
    var logout = createElement("button", "rcc-bento-modal__action rcc-bento-modal__action--danger", "Log Out");

    manage.type = "button";
    logout.type = "button";
    manage.addEventListener("click", function () {
      closeBentoModal();
      activateAccountAction("manage");
    });
    logout.addEventListener("click", function () {
      closeBentoModal();
      activateAccountAction("logout");
    });
    actions.appendChild(manage);
    actions.appendChild(logout);
    openBentoModal("Account", actions);
  }

  function openNotificationModal() {
    var control = findNativeControl("notifications");
    var attempts = 0;

    if (!control) {
      if (window.rccTrace) window.rccTrace("portal:notification-control-missing");
      return;
    }

    if (!document.querySelector(".nav-container .notification-height")) control.click();

    function mountPanel() {
      var panel = document.querySelector(".nav-container .notification-height");
      var placeholder;
      var parent;

      attempts += 1;
      if (!panel && attempts < 8) {
        window.setTimeout(mountPanel, 60);
        return;
      }
      if (!panel) {
        if (window.rccTrace) window.rccTrace("portal:notification-panel-missing");
        return;
      }

      parent = panel.parentNode;
      placeholder = document.createComment("rcc-notification-panel-placeholder");
      parent.insertBefore(placeholder, panel);
      panel.classList.add("rcc-bento-native-notifications");
      openBentoModal("Notifications", panel, function () {
        panel.classList.remove("rcc-bento-native-notifications");
        if (placeholder.parentNode) placeholder.parentNode.replaceChild(panel, placeholder);
        window.setTimeout(function () {
          var trigger = findNativeControl("notifications");
          if (trigger && document.querySelector(".nav-container .notification-height")) {
            trigger.click();
          }
        }, 0);
      });
  ...
  function buildDashboard() {
    ...
    root.addEventListener("click", function (event) {
      ...
      if (accountAction) {
        if (accountAction.dataset.rccAccountAction === "manage") {
          openAccountModal();
        } else {
      ...
      if (actionCard && actionCard.dataset.rccAction === "notifications") {
