(function () {
  ...
  var manualGroupSwitch = false;
  var ACCESS_STATE_KEY = "rcc:community-access-state";
  var lastCommunityURL = window.location.href;
  var ACCELERATOR_UPSELL_ID = "rcc-accelerator-upsell-card";
  var ACCELERATOR_OFFER_URL =
    "https://go.residentialcarecollective.com/accelerator" +
    "?utm_source=rcc_community_portal" +
    "&utm_medium=referral" +
    "&utm_campaign=feasibility_sprint_accelerator_upgrade" +
    "&utm_content=learning_tab_upsell_card";

  function loadObservedAccess() {
    var stored;
    try {
      stored = window.localStorage.getItem(ACCESS_STATE_KEY);
      if (!stored) stored = window.sessionStorage.getItem(ACCESS_STATE_KEY);
  ...
  function rememberObservedAccess(slug, locked) {
    var access;
    if (!slug) return;
    access = loadObservedAccess();
    access[slug] = locked ? "locked" : "unlocked";
    try {
      window.localStorage.setItem(ACCESS_STATE_KEY, JSON.stringify(access));
      window.sessionStorage.setItem(ACCESS_STATE_KEY, JSON.stringify(access));
    } catch (error) {
      if (window.rccTrace) window.rccTrace("community:access-state-storage-failed");
    }
  }

  function observedLockedState(slug, fallback) {
    var value = loadObservedAccess()[slug];
    if (value === "locked") return true;
    if (value === "unlocked") return false;
    /* GHL does not reliably publish inactive membership; unknown starts locked. */
    return true;
  }
  ...
  function groupSlugFromURL(value) {
    var match;
    try {
      match = new URL(value, window.location.href).pathname.match(/\/groups\/([^/?#]+)/);
    } catch (error) {
      return "";
    }
    return match ? decodeURIComponent(match[1]).toLowerCase() : "";
  ...
  function isGroupChange(fromURL, toURL) {
    return groupSlugFromURL(fromURL) !== groupSlugFromURL(toURL);
  }
  ...
  function groupButtonIsLocked(button) {
    var neighborhood = button && button.parentElement;
    var descriptor = [
      ...
      neighborhood && neighborhood.getAttribute("aria-label"),
      neighborhood && neighborhood.getAttribute("title"),
      neighborhood && neighborhood.getAttribute("data-status"),
      neighborhood && neighborhood.getAttribute("data-access"),
      neighborhood && textOf(neighborhood)
    ].filter(Boolean).join(" ");
    var lockIndicator = neighborhood && neighborhood.querySelector(
  ...
  function rememberLiveGroupAccess(button) {
    var slug = groupButtonSlug(button);
    var descriptor = [
      button && button.getAttribute("data-status"),
      button && button.getAttribute("data-access"),
      button && button.getAttribute("aria-label"),
      button && button.parentElement && textOf(button.parentElement)
    ].filter(Boolean).join(" ");

    if (groupButtonIsLocked(button)) {
      rememberObservedAccess(slug, true);
    } else if (/(joined|member|leave group|open group)/i.test(descriptor)) {
      rememberObservedAccess(slug, false);
    }
  }
  ...
  function updateActiveCommunityGroups() {
    ...
    liveButtons.forEach(rememberLiveGroupAccess);
    ...
    rows.forEach(function (row) {
      ...
      var locked = observedLockedState(rowSlug, groupButtonIsLocked(liveButton));
  ...
  function buildGroupRow(button, index) {
    ...
    var locked = observedLockedState(sourceSlug, groupButtonIsLocked(button));
    ...
    row.addEventListener("click", function () {
      ...
      manualGroupSwitch = true;
      window.__rccSkipNextNavigationIntent = true;
  ...
  function findSidebarSupplement(insertPoint) {
    return (
      insertPoint.querySelector(":scope > .flex-1:not(.rcc-community-group-list)") ||
      insertPoint.querySelector(".rcc-community-sidebar-supplement")
    );
  }

  function parkSidebarSupplement(list, insertPoint) {
    var supplement;

    if (!list || !insertPoint || !insertPoint.classList.contains("sidebar")) return;
    supplement = findSidebarSupplement(insertPoint);
    if (!supplement || supplement === list) return;

    supplement.classList.add("rcc-community-sidebar-supplement");
    if (supplement.parentElement !== list) list.appendChild(supplement);
  }

  function preserveSidebarSupplement(list, insertPoint) {
    var supplement = list && list.querySelector(":scope > .rcc-community-sidebar-supplement");
    if (supplement && insertPoint) insertPoint.appendChild(supplement);
  }
  ...
  function enhanceCommunityGroups(force) {
    ...
    groupButtons.forEach(rememberLiveGroupAccess);

    signature = groupButtons.map(function (button) {
      return [
        groupButtonName(button),
        groupButtonSlug(button),
        observedLockedState(groupButtonSlug(button), groupButtonIsLocked(button))
      ].join(":");
    }).join("|");
    ...
    Array.prototype.slice.call(
      ...
      if (existingList.parentElement !== insertPoint) {
        preserveSidebarSupplement(existingList, insertPoint);
    ...
    if (
      ...
      parkSidebarSupplement(existingList, insertPoint);
    ...
    parkSidebarSupplement(list, insertPoint);
    updateActiveCommunityGroups();
  }
  ...
  function isFeasibilitySprintLearningRoute() {
    return /^\/communities\/groups\/rcc-feasibility-sprint\/learning\/?$/.test(
      window.location.pathname
    );
  }

  function findLearningGrid() {
    return document.querySelector(".communities-preview .courses-grid, .courses-grid");
  }

  function updateNativeAcceleratorCard(hidden) {
    var grid = findLearningGrid();
    if (!grid) return;

    Array.prototype.slice.call(grid.children).forEach(function (item) {
      var title = item.querySelector(".title-text");
      var isAccelerator = /accelerator/i.test(textOf(title));

      if (!isAccelerator || item.id === ACCELERATOR_UPSELL_ID) return;
      item.classList.toggle("rcc-native-accelerator-course-source", hidden);
      if (hidden) {
  ...
  function buildAcceleratorUpsellCard() {
    ...
    card.id = ACCELERATOR_UPSELL_ID;
    card.className = "rcc-accelerator-upsell-card";
    card.href = ACCELERATOR_OFFER_URL;
    card.target = "_blank";
    card.rel = "noopener noreferrer";
    card.setAttribute("aria-labelledby", "rcc-accelerator-upsell-title");

    media.className = "rcc-accelerator-upsell-card__media";
    image.src =
      "https://cdn.courses.apisystem.tech/memberships/5wj37F6YjpjxByOejk7F/" +
      "courses/Ownership-Accelerator-Thumbnail-1280x720-ayh5.png";
    image.alt = "Ownership Accelerator";
    image.loading = "lazy";
    media.appendChild(image);

    body.className = "rcc-accelerator-upsell-card__body";
    eyebrow.className = "rcc-accelerator-upsell-card__eyebrow";
    eyebrow.textContent = "Your next step";
    title.id = "rcc-accelerator-upsell-title";
    title.className = "rcc-accelerator-upsell-card__title";
    title.textContent = "Upgrade to Ownership Accelerator™";
    copy.className = "rcc-accelerator-upsell-card__copy";
    copy.textContent =
      "Move from feasibility to a business built to launch, operate, and grow.";
    cta.className = "rcc-accelerator-upsell-card__cta";
    cta.textContent = "Learn More";
    arrow.className = "rcc-accelerator-upsell-card__arrow";
  ...
  function enhanceAcceleratorUpsell() {
    var grid = findLearningGrid();
    var card = document.getElementById(ACCELERATOR_UPSELL_ID);
    var shouldShow = isFeasibilitySprintLearningRoute();

    if (!shouldShow || !grid) {
      if (card) card.remove();
      updateNativeAcceleratorCard(false);
      return;
    }

    updateNativeAcceleratorCard(true);
    if (!card) card = buildAcceleratorUpsellCard();
    if (card.parentElement !== grid || card !== grid.firstElementChild) {
  ...
  function patchHistoryRouting() {
    ...
    ["pushState", "replaceState"].forEach(function (method) {
      ...
      window.history[method] = function () {
        ...
        if (arguments.length > 2 && arguments[2] != null) {
          try {
            targetURL = new URL(String(arguments[2]), window.location.href).href;
            routeChanged = targetURL !== window.location.href;
            groupChanged = isGroupChange(window.location.href, targetURL);
          } catch (error) {
            routeChanged = false;
          }
        }
        if (routeChanged && groupChanged) {
          startCommunityLoading();
        } else if (routeChanged && window.rccTrace) {
          window.rccTrace("community:native-skeleton-route", {
            method: method,
            target: targetURL
          });
        }
        var result = original.apply(this, arguments);
        lastCommunityURL = window.location.href;
    ...
    window.addEventListener("popstate", function () {
      var targetURL = window.location.href;
      if (isGroupChange(lastCommunityURL, targetURL)) {
        startCommunityLoading();
      } else if (window.rccTrace) {
        window.rccTrace("community:native-skeleton-route", {
          method: "popstate",
          target: targetURL
        });
      }
      lastCommunityURL = targetURL;
      updateCurrentCommunityGroup();
      updatePrivateGroupPage();
      updateActiveCommunityGroups();
      scheduleEnhance();
  ...
  function restoreJoinedGroupInteraction(isUnjoined) {
    var scopes;

    if (isUnjoined) return;

    scopes = Array.prototype.slice.call(
      document.querySelectorAll(
        ".communities-top-nav, #post-container, .featured_post_card, .rcc-feed-main"
      )
    );

    scopes.forEach(function (scope) {
      scope.removeAttribute("inert");
      scope.removeAttribute("aria-disabled");
      scope.style.removeProperty("pointer-events");
      scope.querySelectorAll(
        "[inert], .rcc-muted-community-tab, [aria-disabled='true']"
      ).forEach(function (node) {
        node.removeAttribute("inert");
        node.classList.remove("rcc-muted-community-tab");
        if (
          node.matches(
            "a, button, input, textarea, select, [role='button'], [aria-label='Tab Switcher']"
          )
        ) {
  ...
  function updatePrivateGroupPage() {
    ...
    var joinButton = document.getElementById("joinGroup__btn");
    var isPrivateGroup = Boolean(
      isVisiblyRendered(joinButton) &&
      document.getElementById("container_group_about")
    );
    var slug = currentGroupSlug();
    var isJoinedGroup = Boolean(
      slug &&
      document.getElementById("group_info_name") &&
      document.querySelector(".communities-top-nav") &&
      !isPrivateGroup
    );
    var isGroupAbout = /\/communities\/groups\/[^/]+\/about\/?$/.test(
      window.location.pathname
    );
    var isGroupMembers = /\/communities\/groups\/[^/]+\/member-list\/?$/.test(
      window.location.pathname
    );

    if (isPrivateGroup) rememberObservedAccess(slug, true);
    if (isJoinedGroup) rememberObservedAccess(slug, false);

    document.body.classList.toggle("rcc-private-group-page", isPrivateGroup);
    document.body.classList.toggle("rcc-group-about-page", isGroupAbout);
    document.body.classList.toggle("rcc-group-members-page", isGroupMembers);
    updateUnjoinedCommunityControls(isPrivateGroup);
    restoreJoinedGroupInteraction(isPrivateGroup);
    ensureDiscoveryCard(isPrivateGroup);
  }

  function installInteractionDiagnostics() {
    if (document.documentElement.dataset.rccInteractionDiagnostics === "true") return;
    document.documentElement.dataset.rccInteractionDiagnostics = "true";

    document.addEventListener("pointerdown", function (event) {
      var intended = event.target && event.target.closest
        ? event.target.closest(
          ".communities-top-nav, #post-container, .featured_post_card, .post_card"
        )
        : null;
      var topmost;
      var style;
      ...
      style = topmost ? window.getComputedStyle(topmost) : null;
      if (window.rccTrace) {
        window.rccTrace("community:interaction-hit", {
          intended: intended.id || intended.className || intended.tagName,
          target: event.target.id || event.target.className || event.target.tagName,
          topmost: topmost && (topmost.id || topmost.className || topmost.tagName),
          ...
          unjoined: document.body.classList.contains("rcc-unjoined-community-page")
        });
      }
    }, true);
  }
  ...
  function markSurfaceReady() {
    ...
    function checkReady() {
      ...
      if ((bootstrapFromCommunities || manualGroupSwitch) && isGroupPath) {
      ...
      if (window.rccTrace) {
        window.rccTrace("community:ready-check", {
          ...
          manualGroupSwitch: manualGroupSwitch,
