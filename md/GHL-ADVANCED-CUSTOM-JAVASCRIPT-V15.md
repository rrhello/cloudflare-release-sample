# GHL Advanced Custom JavaScript Loader — v15 Diagnostics

Paste this into GHL Advanced Custom JavaScript without `<script>` tags. Replace
`PREVIEW_HOST` with the immutable v15 QA host.
...
(function () {
  ...
  var release = "v15";
  var releaseBase = "https://PREVIEW_HOST.rccportal.pages.dev/releases/v15/";
...
Keep the v14 initial-paint Custom CSS guard. After reproducing a transition,
...
Copy everything between `RCC_DEBUG_REPORT_START` and `RCC_DEBUG_REPORT_END`.
The report records route and loader timing but does not collect page text,
cookies, localStorage, form values, or authentication tokens.
